...
from database import (
    ...
    get_monthly_voice_sessions,
    get_monthly_member_stats,
    get_annual_voice_sessions,
    get_annual_member_total_stats,
    get_participants_by_session_ids,
    get_total_call_time_for_guild_members,
    get_mute_count,
    get_monthly_mute_counts,
...
class BotCommands(commands.Cog):
    ...
    # --- /stats コマンドグループ ---
    stats = app_commands.Group(name="stats", description="通話統計に関するコマンド")
    ...
    async def _get_monthly_statistics(self, guild, month: str):
        ...
        if not sessions_data:
            ...
            # 月間平均通話時間の計算
            monthly_avg = sum(sess["duration"] for sess in sessions_data) / len(
                sessions_data
            )
            ...
            longest_date = formatters.convert_utc_to_jst(
                datetime.datetime.fromisoformat(longest_session["start_time"])
            ).strftime("%Y/%m/%d")
            ...
            longest_participants = participants_map.get(
                longest_session_id, []
            )  # 参加者リストを取得

            longest_participants_names = self._get_member_display_names(
                guild, longest_participants
            )
        ...
        ranking_display_names = self._get_member_display_names(
            guild, member_ids_in_ranking
        )

        for i, (member_id, duration) in enumerate(sorted_members, start=1):
            name = ranking_display_names[i - 1]  # ヘルパー関数で取得した表示名を使用
            ranking_lines.append(
                f"{i}.  {formatters.format_duration(duration)}  {name}"
            )
    ...
    async def _create_monthly_stats_embed(
        ...
        # Embedを作成し、フィールドを追加
        embed = discord.Embed(
            title=f"【{month_display}】通話統計情報",
            color=constants.EMBED_COLOR_SUCCESS,
        )
        embed.add_field(
            name=constants.EMBED_FIELD_AVERAGE_CALL_TIME,
            value=f"{formatters.format_duration(monthly_avg)}",
            inline=False,
        )
        embed.add_field(
            name=constants.EMBED_FIELD_LONGEST_CALL, value=longest_info, inline=False
        )
        embed.add_field(
            name=constants.EMBED_FIELD_CALL_RANKING, value=ranking_text, inline=False
        )
        embed.add_field(
            name=constants.EMBED_FIELD_MUTE_RANKING,
            value=mute_ranking_text,
            inline=False,
        )
    ...
    # --- 月間ミュート回数ランキング作成用ヘルパー関数 ---
    async def _get_monthly_mute_ranking(self, guild, month: str):
        logger.info(
            f"Fetching monthly mute ranking for guild {guild.id}, month {month}"
        )
        mute_counts = await get_monthly_mute_counts(
            month
        )  # データベースから直接リストを取得

        # get_monthly_mute_countsはリストのタプルを返すため、items()は不要
        sorted_mutes = sorted(mute_counts, key=lambda x: x[1], reverse=True)

        mute_ranking_lines = []
        # member_ids_in_ranking はタプルの最初の要素 (user_id) を使用
        mute_member_ids_in_ranking = [user_id for user_id, count in sorted_mutes]
        mute_ranking_display_names = self._get_member_display_names(
            guild, mute_member_ids_in_ranking
        )

        for i, (member_id, count) in enumerate(
            sorted_mutes[: constants.RANKING_LIMIT], start=1
        ):
            name = mute_ranking_display_names[i - 1]
            mute_ranking_lines.append(f"{i}.  {count} 回  {name}")
        mute_ranking_text = (
            "\n".join(mute_ranking_lines) if mute_ranking_lines else "なし"
        )
        logger.debug(f"Monthly mute ranking text generated:\n{mute_ranking_text}")
        return mute_ranking_text
    ...
    async def get_and_process_annual_stats_data(self, guild, year: str):
        ...
        for i, (member_id, duration) in enumerate(sorted_members, start=1):
            name = ranking_display_names[i - 1]
            ranking_lines.append(
                f"{i}.  {formatters.format_duration(duration)}  {name}"
            )
        ranking_text = "\n".join(ranking_lines) if ranking_lines else "なし"
    ...
    async def _create_annual_stats_embed(
        self,
        year_display: str,
        avg_duration: float,
        longest_info: str,
        ...
        embed.add_field(
            name="年間: 平均通話時間",
            value=f"{formatters.format_duration(avg_duration)}",
            inline=False,
        )
        embed.add_field(name="年間: 最長通話", value=longest_info, inline=False)
        embed.add_field(
            name="年間: 通話時間ランキング", value=ranking_text, inline=False
        )
        logger.debug("Annual stats embed created successfully.")
        # 作成したEmbedを返す
        return embed

    # --- /stats monthly サブコマンド ---
    @stats.command(
        name="monthly",
        description="指定した月の通話統計と寝落ちミュート回数ランキングを表示します。",
    )
    ...
    async def stats_monthly(self, interaction: discord.Interaction, month: str = None):
        ...
        try:
            ...
            await interaction.followup.send(
                "指定された月の形式が正しくありません。形式は YYYY-MM で指定してください。",
                ephemeral=True,
            )
            return
        ...
        if embed is None:
            ...
            await interaction.followup.send(
                f"{month_display}は通話統計情報が記録されていません", ephemeral=True
            )
            return
    ...
    @stats.command(
        name="total",
        description="指定したメンバーの総通話時間と寝落ちミュート回数を表示します。",
    )
    @app_commands.describe(member="表示したいメンバーを選択してください")
    @app_commands.guild_only()
    async def stats_total(
        ...
        # 累計ミュート回数の取得と表示 (既存の /get_mute_count ロジックを移植)
        ...
        if total_seconds == 0:
            embed.add_field(
                name=constants.EMBED_FIELD_TOTAL_CALL_TIME,
                value=constants.MESSAGE_NO_CALL_HISTORY,
                inline=False,
            )
        else:
            embed.add_field(
    ...
    # --- /stats ranking サブコマンド ---
    @stats.command(
        name="ranking",
        description="総通話時間と寝落ちミュート回数のランキングを表示します。",
    )
    @app_commands.guild_only()
    async def stats_ranking(self, interaction: discord.Interaction):
        ...
        # 総通話時間ランキングの取得と表示 (既存の /call_ranking ロジックを移植)
        logger.debug(
            f"Fetching total call times for {len(members)} members in guild {guild.id}"
        )
        ...
        filtered_member_call_times = {
            member_id: total_seconds
            for member_id, total_seconds in member_call_times.items()
            if total_seconds > 0
        }
        logger.debug(
            f"Found call times for {len(filtered_member_call_times)} members with > 0 call time."
        )

        sorted_call_members = sorted(
            filtered_member_call_times.items(), key=lambda x: x[1], reverse=True
        )
        logger.debug(f"Sorted {len(sorted_call_members)} members for call ranking.")

        call_ranking_text = ""
        if not sorted_call_members:
            call_ranking_text = constants.MESSAGE_NO_RANKING_DATA
            logger.info("No call ranking data found.")
        else:
            member_lookup = {member.id: member for member in guild.members}
            for i, (member_id, total_seconds) in enumerate(
                sorted_call_members[: constants.RANKING_LIMIT], start=1
            ):
                member = member_lookup.get(member_id)
                if member:
                    formatted_time = formatters.format_duration(total_seconds)
                    call_ranking_text += (
                        f"{i}. {formatted_time} {member.display_name}\n"
                    )
            if len(sorted_call_members) > constants.RANKING_LIMIT:
                call_ranking_text += f"...\n(上位 {constants.RANKING_LIMIT} 名を表示)"
            logger.info(
                f"Call ranking text generated, showing top {min(len(sorted_call_members), constants.RANKING_LIMIT)}."
            )

        # 累計ミュート回数ランキングの取得と表示
        mute_counts = await get_total_mute_counts()
        mute_ranking_text = ""
        if mute_counts:
            sorted_mute_members = sorted(mute_counts, key=lambda x: x[1], reverse=True)
            mute_ranking_text = ""
            member_lookup = {member.id: member for member in guild.members}
            for i, (user_id, count) in enumerate(
                sorted_mute_members[: constants.RANKING_LIMIT], start=1
            ):
                user = member_lookup.get(user_id) or await self.bot.fetch_user(user_id)
                if user:
                    mute_ranking_text += f"{i}. {count} 回 {user.display_name}\n"
            if len(sorted_mute_members) > constants.RANKING_LIMIT:
                mute_ranking_text += f"...\n(上位 {constants.RANKING_LIMIT} 名を表示)"
            logger.info(
                f"Mute ranking text generated, showing top {min(len(sorted_mute_members), constants.RANKING_LIMIT)}."
            )
        else:
            mute_ranking_text = "まだ寝落ちミュートされたデータがありません。"
            logger.info("No mute ranking data found.")
        ...
        embed.add_field(
            name="総通話時間ランキング", value=call_ranking_text, inline=False
        )
        embed.add_field(
    ...
    async def stats_current(self, interaction: discord.Interaction):
        ...
        if not active_calls:
            ...
            await interaction.followup.send(
                constants.MESSAGE_NO_ACTIVE_CALLS, ephemeral=True
            )
        else:
            ...
            for call in active_calls:
                ...
                if channel:
                    embed.add_field(
                        name=f"{channel.name}", value=call["duration"], inline=False
                    )
                else:
                    logger.warning(
                        f"Channel with ID {channel_id} not found in bot cache."
                    )
                    embed.add_field(
                        name=f"不明なチャンネル ({channel_id})",
                        value=call["duration"],
                        inline=False,
                    )
    ...
    async def help_callback(self, interaction: discord.Interaction):
        ...
        # コマンド一覧表示用のEmbedを作成
        embed = discord.Embed(
            title=constants.EMBED_TITLE_COMMAND_LIST,
            color=constants.EMBED_COLOR_SUCCESS,
        )
    ...
    @app_commands.command(
        name="changesendchannel",
        description="通知を送信するチャンネルを設定します（管理者用）",
    )  # nameとdescriptionを明示
    ...
    async def changesendchannel_callback(
        ...
        logger.debug(
            f"Current notification channel ID for guild {guild_id}: {current_channel_id}"
        )
        ...
        if current_channel_id is not None and current_channel_id == channel.id:
            ...
            await interaction.response.send_message(
                constants.MESSAGE_NOTIFICATION_CHANNEL_ALREADY_SET.format(
            ...
            logger.info(
                f"Notification channel set to {channel.id} for guild {guild_id}"
            )
            await interaction.response.send_message(
                constants.MESSAGE_NOTIFICATION_CHANNEL_SET.format(channel=channel),
                ephemeral=True,
            )
    ...
    async def debug_annual_stats_callback(
        ...
        (
            ...
            sessions_data,
            members_total,
        ) = await self.get_and_process_annual_stats_data(interaction.guild, year)
        ...
        if sessions_data:
            ...
            await interaction.response.send_message(
                f"{year_display}{constants.MESSAGE_NO_CALL_RECORDS}", ephemeral=True
            )
    ...
    @app_commands.command(
        name="set_sleep_check", description="寝落ち確認の設定を変更します（管理者用）"
    )  # nameとdescriptionを明示
    @app_commands.default_permissions(administrator=True)  # 管理者権限が必要
    @app_commands.describe(
        lonely_timeout_minutes="一人以下の状態が続く時間（分単位）",
        reaction_wait_minutes="反応を待つ時間（分単位）",
    )
    @app_commands.guild_only()
    async def set_sleep_check_callback(
        ...
        if lonely_timeout_minutes is None and reaction_wait_minutes is None:
            ...
            logger.info(
                f"Displaying current sleep check settings for guild {interaction.guild.id}"
            )
            await interaction.response.send_message(
        ...
        if lonely_timeout_minutes is not None and lonely_timeout_minutes <= 0:
            logger.warning(
                f"Invalid lonely_timeout_minutes value: {lonely_timeout_minutes}"
            )
            await interaction.response.send_message(
                constants.MESSAGE_LONELY_TIMEOUT_MIN_ERROR, ephemeral=True
            )
            return
        if reaction_wait_minutes is not None and reaction_wait_minutes <= 0:
            logger.warning(
                f"Invalid reaction_wait_minutes value: {reaction_wait_minutes}"
            )
            await interaction.response.send_message(
                constants.MESSAGE_REACTION_WAIT_MIN_ERROR, ephemeral=True
            )
            return
        ...
        await update_guild_settings(
            ...
            lonely_timeout_minutes=lonely_timeout_minutes,
            reaction_wait_minutes=reaction_wait_minutes,
        )
